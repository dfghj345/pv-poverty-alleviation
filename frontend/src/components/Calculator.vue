...
<template>
  <StableQueryCard class="business-flip-card--large" :active="panelSide === 'result' ? 'back' : 'front'">
    <template #front>
      <div class="business-card-scroll">
        ...
        <form class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2" @submit.prevent="handleSubmit">
          <div>
          ...
          <div class="business-card-actions sm:col-span-2">
            <button type="submit" :disabled="calculating" class="apple-pill-primary flex items-center gap-2 text-base">
              <svg v-if="calculating" class="h-5 w-5 animate-spin text-white" fill="none" viewBox="0 0 24 24">
        ...
        <div v-if="errorMsg" class="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-400">
          {{ errorMsg }}
        </div>
      </div>
    </template>
    ...
    <template #back>
      <div class="business-card-scroll business-card-scroll--result">
        ...
        <div class="business-result-body">
          ...
          <div class="relative mt-4 h-[320px] w-full overflow-hidden rounded-2xl border border-emerald-100/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.94),rgba(240,253,250,0.9))] dark:border-slate-700 dark:bg-dark-card sm:h-[380px]">
            <div
              ref="chartRef"
