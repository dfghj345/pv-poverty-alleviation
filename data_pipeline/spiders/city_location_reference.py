...
class CityLocationReferenceSpider(BaseSpider[dict, CityLocationRawItem]):
    ...
    async def fetch(self, ctx: RunContext) -> dict:
        ...
        if is_placeholder_url(self._site_url):
            default_items = _load_seed_file(DEFAULT_CITY_LOCATION_SEED_PATH, log=log, warn_missing=True)
            if default_items:
                log.warning('city location real data source is not configured; using seed fallback', extra={'seed_path': str(DEFAULT_CITY_LOCATION_SEED_PATH), 'url': self._site_url})
                return {'items': default_items}
            log.warning('city location seed fallback is missing', extra={'seed_path': str(DEFAULT_CITY_LOCATION_SEED_PATH), 'url': self._site_url})
...
def _load_seed_file(path: Path, *, log, warn_missing: bool = False) -> list[dict[str, Any]]:
    if not path.exists():
        if warn_missing:
            log.warning('city location seed file missing', extra={'path': str(path)})
        return []
    try:
